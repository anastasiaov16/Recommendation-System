...
#        a = movies_tb[movies_tb['movieId'] == rek[p]]['genres'].values
#        if len(a)!=0:
#            b = a[0].replace('|', ' ').split()
#        for j in range(0, len(b)):
#            if(b[j] == key[0] or b[j] == key[1] or b[j] == key[2]):
#                if movies_tb['movieId'].iloc[rek[p]] not in mov:
#                    mov.append(movies_tb['movieId'].iloc[rek[p]])
#    df[t]=mov
...
#print(df)
#pd.DataFrame.from_dict(data=df, orient='index').to_csv('hybryd.csv', header=False)
